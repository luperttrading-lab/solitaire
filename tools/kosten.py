...
# Textanfaenge, die das Geruest erzeugt und die keine Frage von Lutz sind.
# `<` und `[` decken die meisten ab (`<task-notification>`, `<system-reminder>`,
# `<wake …>`, `[SYSTEM NOTIFICATION …]`) - diese hier nicht.
#
# Gefunden am 20.09.2026, nachdem Lutz an einer Arbeitszeit von 2 min
# zweifelte: die war richtig, aber die Suche fand `Stop hook feedback:`.
# Dieser Eintrag stand um 10:51:03 mitten in der Arbeit an v1.60 und setzte
# den Schnitt neu. Gemessen: die Zeile um 11:08 zeigte „Frage 2,15" statt
# der wahren 6,55 $ seit 10:39:51 - **4,40 $ verschwiegen**, also zwei
# Drittel. Genau derselbe Fehlertyp wie der, den Lutz am 16.09. fand; eine
# Liste ist die ehrlichere Form als zwei Sonderfaelle im Ausdruck.
GERUEST = (
...
def echt(text):
    ...
    t = (text or '').strip()
    return bool(t) and t[0] not in '<[' and not t.startswith(GERUEST)
...
if '-v' in sys.argv:
    # Damit die Arbeitszeit nachprüfbar ist: wo der Schnitt liegt, steht hier.
    st = zeitpunkt(last_user)
    print('Frage ab:', st.astimezone(ZONE).strftime('%d.%m. %H:%M:%S') if st else '-',
          '| letzter Eintrag:',
          (zeitpunkt(last_any).astimezone(ZONE).strftime('%H:%M:%S')
           if zeitpunkt(last_any) else '-'))
